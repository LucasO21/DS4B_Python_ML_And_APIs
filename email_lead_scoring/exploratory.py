...
def explore_sales_by_category(data, category="country_code", sort_by=["sales", "prop_in_group"]):
    """This function explores the 'made_purchase' column by a cateogorical column. 
...
def explore_sales_by_numeric(data, numeric=["tag_count"], q = [0.10, 0.50, 0.90]):
    """This function explores the subscriber data using the column 'made_purchase' and any numeric column.

    Args:
        data (DataFrame): The subscriber data with a column added 'made_purchase'.
        numeric (list, optional, str): One or more. Defaults to ["tag_count"].
